...
int admin_dashboard() {
    ...
    int acc_count = 0, loan_count = 0;
    int active = 0, active_loans = 0;
    ...
    for (int i = 0; i < loan_count; i++) {
        if (loans[i].status == LOAN_ACTIVE) {
            active_loans++;
            loan_balance += loans[i].remaining_balance;
        }
    }
...
int admin_view_account() {
    ...
    Loan loans[MAX_LOANS];
    int txn_count = 0, loan_count = 0;
    int found = 0;

    print_header("VIEW ACCOUNT DETAILS");
    int id = get_int(" Enter account ID: ", STARTING_ACCOUNT_ID, 99999);

    if (!find_account(id, &acc)) {
    ...
    found = 0;
    for (int i = txn_count - 1; i >= 0 && found < 10; i--) {
        if (txns[i].account_id == id) {
            print_transaction_row(&txns[i]);
            found++;
        }
    }
    ...
    found = 0;
    for (int i = 0; i < loan_count; i++) {
        if (loans[i].account_id == id) {
            print_loan_row(&loans[i]);
            found++;
        }
    }
...
int admin_toggle_account() {
    ...
    char prompt[32];
    snprintf(prompt, sizeof(prompt), " Type YES to %s: ", acc.is_active ? "deactivate" : "activate");
    get_string(prompt, confirm, sizeof(confirm));
