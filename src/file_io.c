...
// file or directory", "Permission denied", etc.) instead of us guessing.
...
int load_accounts(Account *arr, int *count) {
    ...
    *count = 0;
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (n < 9) continue; // malformed line, just skip it
        a->type = (AccountType)type;
        a->tier = (n >= 12) ? (CustomerType)tier : NORMAL;   // default older records to Normal
        a->is_active = active;
        a->failed_attempts = failed;
        a->withdrawn_today = (n >= 12) ? withdrawn : 0.0;
        strncpy(a->last_withdraw_date, (n >= 12) ? last_withdraw : a->created_at, MAX_DATE_LEN - 1);
        (*count)++;
    }
    fclose(fp);
    return 1;
...
// finds the account by id in the full list, replaces it, rewrites the whole file
// (yeah it's not efficient but the file is tiny so it doesn't matter here)
int update_account(const Account *updated) {
    Account arr[MAX_ACCOUNTS];
    int count = 0, i;
    if (!load_accounts(arr, &count)) return 0;
    for (i = 0; i < count; i++) {
        if (arr[i].id == updated->id) { arr[i] = *updated; return save_accounts(arr, count); }
    }
    return 0;
}
...
int append_transaction(const Transaction *txn) {
    ...
    if (!fp) {
        ...
        // file doesn't exist yet, create it with the header comment first
        errno = 0;
...
int update_loan(const Loan *updated) {
    ...
    for (i = 0; i < count; i++) {
        if (arr[i].id == updated->id) { arr[i] = *updated; return save_loans(arr, count); }
    }
    return 0;
}
